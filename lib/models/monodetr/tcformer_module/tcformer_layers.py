...
from .tcformer_utils import (
    ...
    map2token, token_downup, index_points)
...
class CTM(nn.Module):
    def __init__(self, sample_ratio, embed_dim, dim_out, k=5):
        ...
        self.k = k
        self.score_net = nn.Sequential(nn.Conv2d(dim_out, dim_out, kernel_size=3, padding=1, bias=True),
                                     nn.ReLU(inplace=True),
                                     nn.Conv2d(dim_out, 1, kernel_size=3, stride=1, padding=1, bias=True))
    ...
    def cluster_dpc_knn(self, token_dict, cluster_num, k=5, token_mask=None):
        ...
        m = int((N//120)**0.5)
        W, H = 20*m, 6*m
        window_w, window_h = 2.5*m, 1*m

        index_down = token_dict['indexs']
        idx_value = index_points(x, index_down)
        dist_matrix = torch.cdist(idx_value,x)
    ...
    def forward(self, token_dict):
        ...
        idx_cluster, cluster_num = self.cluster_dpc_knn(
